...
describe("ThemeContext", () => {
  ...
  it("prefers a previously saved choice over the server default", () => {
    window.localStorage.setItem("restly-theme", "light");
    defaultTheme = "dark";
    renderTheme();
  ...
  it("updates the mode and persists it when setMode is called", async () => {
    ...
    expect(screen.getByTestId("mode")).toHaveTextContent("dark");
    expect(screen.getByTestId("resolved")).toHaveTextContent("dark");
    expect(window.localStorage.getItem("restly-theme")).toBe("dark");

    await user.click(screen.getByText("set-light"));
    expect(screen.getByTestId("resolved")).toHaveTextContent("light");
    expect(window.localStorage.getItem("restly-theme")).toBe("light");
  });
